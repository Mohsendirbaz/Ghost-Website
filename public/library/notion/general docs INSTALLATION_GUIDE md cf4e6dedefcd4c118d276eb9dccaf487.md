# general docs\INSTALLATION_GUIDE.md
...
3. Go to: **File â†’ Settings** (Windows/Linux) or **IntelliJ IDEA â†’ Preferences** (Mac)
4. Navigate to: **Build, Execution, Deployment â†’ Build Tools â†’ Gradle**
...
```bash
# Find any Gradle project on your system and copy:copy "C:\OtherProject\gradle\wrapper\gradle-wrapper.jar" "gradle\wrapper\gradle-wrapper.jar"
```
...
4. **Click the gear icon** (âš™ï¸) next to “Installed”
...
2. Look for the tool window:
    - **View â†’ Tool Windows â†’ AutoAgents**
    - Or check the right sidebar for AutoAgents icon
3. Check the Tools menu:
    - **Tools â†’ AutoAgents** should be visible
...
```bash
# Check which Java Gradle is usinggradlew.bat --version# Should show Java 17 or 21, not 23
```
...
2. Check if plugin is enabled in Settings â†’ Plugins
3. Try invalidating caches: File â†’ Invalidate Caches and Restart
