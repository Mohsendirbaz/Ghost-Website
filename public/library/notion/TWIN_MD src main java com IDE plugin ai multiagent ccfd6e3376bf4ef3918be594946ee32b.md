# TWIN_MD\src\main\java\com.IDE.plugin\ai\multiagent\trust\TrustLevel.md
...
```java
TrustLevel agentTrust = TrustLevel.MEDIUM;int trustValue = agentTrust.getValue(); // Returns 2String description = agentTrust.getDescription(); // Returns "Medium trust"
```

### Converting from Numeric Value

```java
int calculatedTrustValue = 3;TrustLevel level = TrustLevel.fromValue(calculatedTrustValue); // Returns HIGH
```

### Trust Level Comparison

```java
if (agentTrust.getValue() >= TrustLevel.HIGH.getValue()) {    // Grant high-privilege operations}
```
