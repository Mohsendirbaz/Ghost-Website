# general docs\threshold_definitions.md

# threshold_definitions.md
...
**Mathematical Explanation**: RSA security is based on the difficulty of factoring large numbers. A 2048-bit key means the modulus n = pÃ—q is approximately 2^2048, making factorization computationally infeasible with current technology.
...
**Mathematical Explanation**: For each hour of inactivity, the reputation score moves toward the neutral value by DECAY_RATE. If S is the current score and N is the neutral score, the new score after t hours is: S' = S - (S-N) Ã— (1-(1-DECAY_RATE)^t)
...
**Mathematical Explanation**: For each decay cycle, activation is multiplied by (1-ACTIVATION_DECAY_RATE). After n cycles, activation becomes: A' = A Ã— (1-ACTIVATION_DECAY_RATE)^n
...
**Mathematical Explanation**: Uses Jaccard similarity coefficient: J(A,B) = |Aâˆ©B|/|AâˆªB| where A and B are the sets of keywords associated with each concept.
