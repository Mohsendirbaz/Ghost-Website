# 🔶 ACTIVE: Agent Rotational Development Program
...
@dataclassclass DomainExperience:
 ...
 chapter_count: intdef record_activity(self, activity: str, impact: float, timestamp: datetime):
...
if "chapter_complete" in activity:
 self.chapter_count += 1def get_expertise_level(self) -> float:
...
@dataclassclass RotationalAssignment:
 ...
 completed: booldef mark_completed(self):
...
for record in records:
 ...
 weight = 1.0 / (1.0 + days_ago / 30) # Half-life of 30 daysweighted_sum += record.confidence * weight
 total_weight += weight
profile[domain] = weighted_sum / total_weight if total_weight > 0 else 0.0return profile
...
def _calculate_frequency(self, assignments: List) -> float:
 ...
 return 0.0earliest = min(a.timestamp for a in assignments)
 latest = max(a.timestamp for a in assignments)
 years = (latest - earliest).days / 365.0return len(assignments) / years if years > 0 else 0.0@dataclassclass RotationalAssignmentRecord:
...
@dataclassclass RotationalPolicy:
 """Policy defining rotation eligibility criteria""" min_specialty_depth_days: int max_rotation_frequency_per_year: int min_time_between_rotations_days: intdef assess_eligibility(self, specialty_depth_days: int,
